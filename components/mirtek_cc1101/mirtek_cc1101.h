...
class MirtekCC1101 : public PollingComponent,
 ...
 public:
  ...
  void loop() override {
    ...
    // только фактическим временем приёма ответа (как в packetReceiver()).
    if (next != PH_IDLE) start_phase_(next);
  }
 ...
 protected:
  ...
  // Общий бюджет ожидания ответа на одну команду (все подпакеты суммарно).
  // В самом My_Mirtek_Demon.ino стоит TimerMs tmr(10000, 0, 0) — 10000 мс.
  // Здесь сознательно поставлено 2000 мс по прямому указанию владельца
  // системы, на основе его собственного тестирования на реальном
  ...
  int pump_got_{0};
  uint32_t pump_t0_{0};      // старт общего RX-окна команды (лимит RX_TIMEOUT_MS)
  ...
  void begin_command_(uint8_t cmd, int sub1, int sub2, int expected_pkts) {
    ...
    // Полностью повторяем SmartRC SendData(): раздельная запись внешнего
    // аппаратного length-префикса, затем burst всего tx_stuffed_ (который
    // сам начинается с tx_stuffed_[0]=длина — тот самый дублирующий байт,
    // симметричный находке на RX-стороне, см. комментарий в начале файла).
    // Без этой отдельной записи CC1101 использовал бы tx_stuffed_[0]
    // (=len-1) как единственный length-префикс и передавал бы РОВНО на
    // 1 байт меньше — теряя STOP-байт (0x55) в конце каждого пакета.
    // Подтверждено на реальном железе.
  ...
  void pump_tick_() {
    switch (pump_state_) {
      ...
      case PS_RX_WAIT_HIGH: {
        if (millis() - pump_t0_ > RX_TIMEOUT_MS) {
          finish_pump_();
          return;
        }
      ...
      case PS_RX_WAIT_LOW: {
        ...
        } else if (millis() - pump_t0_ > RX_TIMEOUT_MS) {
          finish_pump_();
        }
        break;
      }
